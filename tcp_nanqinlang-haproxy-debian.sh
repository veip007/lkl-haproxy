...
config(){
	...
	[[ ! -f redirect.sh ]] && wget https://raw.githubusercontent.com/veip007/lkl-haproxy/master/requirement/redirect.sh
...
check-all(){
	...
	# check lkl-mod
	[[ ! -f tcp_nanqinlang.so ]] && wget https://raw.githubusercontent.com/veip007/lkl-haproxy/master/mod/tcp_nanqinlang.so
	[[ ! -f tcp_nanqinlang.so ]] && echo -e "${Error} download lkl.mod failed, please check !" && exit 1

	# check lkl-load
	[[ ! -f load.sh ]] && wget https://raw.githubusercontent.com/veip007/lkl-haproxy/master/requirement/load.sh
	...
	# check selfstart
	[[ ! -f start.sh ]] && wget https://raw.githubusercontent.com/veip007/lkl-haproxy/master/requirement/start.sh
